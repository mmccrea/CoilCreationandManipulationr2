#patch.py consists of two primary classes.
#coil - stores a set of points (x,y,z) defining a closed loop that is a electric current path, and some functiosn for modifying itself.
#coilset - a set of coils and a serioes of functions for operating on them as a group
#affine_matrix - a set of functions for performing affine transformations on a coil.
#b_segment, b_loop are functions for calculating magnetic fields on wire segments using the biot-savart law.

# Fri May 24 10:11:45 CDT 2019 Jeff added this line.
# June 16, 2019 Jeff modified to limit to class definitions.
# March 28, 2022 M. McCrea updates were made to this code mainly for plotting and some coil manipulation.
...
class coilset:
    ...
    def filter_coil(self,filter_axis=-1,fmin=0,fmax=0):
        '''
        This method filters on the points of each coil, keeping all points in each coil that meet the filter requirements.  This can cause issues as it can create an overlapping line that did not exist before.
        '''
        for coil in self.coils:
            if filter_axis == 0 or filter_axis == 1 or filter_axis == 2:
                flt = np.logical_and(fmin<=coil.points[:,filter_axis],coil.points[:,filter_axis]<=fmax)
                #unclose the loop as that is not generally required if filtering down to one plane
                if len(flt) !=0: #if there is a filter list
                    if np.any(flt==True): #if any points remain after filtering
                        print("coil.points[flt] = \n" , coil.points[flt])
                        if np.all(coil.points[flt][0] == coil.points[flt][-1]):
                            flt[-1]=False
                ax.plot(coil.points[flt][:,0],coil.points[flt][:,1],coil.points[flt][:,2],**plt_kwargs)
                
            else:
                ax.plot(coil.points[:,0],coil.points[:,1],coil.points[:,2],**plt_kwargs)
                # print("coil.points = " , coil.points)
        ax.set_xlabel('x (m)')
        ax.set_ylabel('y (m)')
        ax.set_zlabel('z (m)')

    def draw_coils(self,ax,filter_axis=-1,fmin=0,fmax=0,**plt_kwargs):
        for coil in self.coils:
            if filter_axis == 0 or filter_axis == 1 or filter_axis == 2:
                flt = np.logical_and(fmin<=coil.points[:,filter_axis],coil.points[:,filter_axis]<=fmax)
                #unclose the loop as that is not generally required if filtering down to one plane
    ...
    def draw_pts(self, ax, points, sorting, equal=True, arrow=True, poslabel=False, fmin=0, fmax=0, **plt_kwargs):
        '''
        ax - matplotlib axis to draw on
        points- n x 3 array of points to be drawn
        sorting = 1x3 arroay indicating columns for [x-axis in plot, y-axis in plot, filter axis for fmin and fmax
        equal - bool if axis should be equal scale (makes circles circular instead of ellispses)
		...
		#if a filter axis is defined, use fmin and fmax to set a range of points to be removed.
        if sorting[2] == 0 or sorting[2] == 1 or sorting[2] == 2:
            pts = points[np.logical_and(fmin<=points[:,sorting[2]],points[:,sorting[2]]<=fmax)]
        else:
            pts = points
        # print("points = " , points)
        # print("pts = " , pts)
        #As using lines is only required for arrows or labels and it can be slow, it will be avoided if not required.
        if arrow or poslabel:
            lines = ax.plot(pts[:,sorting[0]],points[:,sorting[1]],**plt_kwargs)[0]
            if arrow: add_arrow(lines,size=14)
            if poslabel: add_poslabel(lines)
        else:
            ax.plot(pts[:,sorting[0]],pts[:,sorting[1]],**plt_kwargs)[0]
        if equal: ax.axis('equal')
    
    def draw_xy(self,ax,filter_axis=-1,equal=True,arrow=True,poslabel=False,fmin=0,fmax=0,**plt_kwargs):
        for coil in self.coils:
            self.draw_pts(ax,coil.points,[0,1,filter_axis],equal,arrow,poslabel,fmin,fmax,**plt_kwargs)
        ax.set_xlabel('x (m)')
        ax.set_ylabel('y (m)')
    def draw_zy(self,ax,filter_axis=-1,equal=True,arrow=True,poslabel=False,fmin=0,fmax=0,**plt_kwargs):
        for coil in self.coils:
            self.draw_pts(ax,coil.points,[2,1,filter_axis],equal,arrow,poslabel,fmin,fmax,**plt_kwargs)
        ax.set_xlabel('z (m)')
        ax.set_ylabel('y (m)')
    def draw_xz(self,ax,filter_axis=-1,equal=True,arrow=True,poslabel=False,fmin=0,fmax=0,**plt_kwargs):
        for coil in self.coils:
            self.draw_pts(ax,coil.points,[0,2,filter_axis],equal,arrow,poslabel,fmin,fmax,**plt_kwargs)
        ax.set_xlabel('x (m)')
        ax.set_ylabel('z (m)')
    def draw_layout(self,fig, filter_axis=-1, fmin=0, fmax=0 , equal= True,arrow=True,poslabel=False, **plt_kwargs):
        #Note: fmin, fmax filtering is currently only poorly implemented, its results are uneven and create new lines that weren't there before.
        if not fig.axes:
            ax3=[]
        ...
        self.draw_xy(ax3[0],filter_axis,fmin=fmin,fmax=fmax,equal=equal,arrow=arrow,poslabel=poslabel,**plt_kwargs)
        self.draw_zy(ax3[1],filter_axis,fmin=fmin,fmax=fmax,equal=equal,arrow=arrow,poslabel=poslabel,**plt_kwargs)
        self.draw_xz(ax3[2],filter_axis,fmin=fmin,fmax=fmax,equal=equal,arrow=arrow,poslabel=poslabel,**plt_kwargs)
        self.draw_coils(ax3[3],filter_axis,fmin,fmax,**plt_kwargs)
        return ax3
    
    def make_closed(self):
        '''
        search all loops in the coil_set and make the loops closed if the first and last point in the list are not identical.
        '''
        count = 0
        for coil in self.coils:
    ...
    def make_open(self):
        '''
        search all loops in the coil_set and make the loops open if the first and last point in the list are identical.
        '''
        count = 0
        for coil in self.coils:
    ...
    def round_all(self,decimals=6):
        '''
        applies numpy.round to keep only the first 6 decimal places.
        This can be used to prevent floating point errors from causing problems with conversions such as 1.10000000000089 vs 1.1000000000000 while maintinaing precision to 1/1000 of a mm.
        '''
        for coil in self.coils:
            # print("original = " , coil.points)
            coil.points = np.round(coil.points, decimals=6)
            # print("rounded = " , coil.points)
    # def simplify_coils(self,vw):
        # for coil in self.coils:
            
    def output_csv(self,outfolder,outfile,open=False):
        """Outputs a csv file for each loop in a coil into the specified folder.
        If the folder does not exist, it will be created
        If open==True it removes the last point if it is the same as the first point in the loop."""
        try:
            mkdir(outfolder)
        ...
        for number in range(self.ncoils):
            ...
            points = coil.points
            if open:
            #remove last point from list of points without modifying the coil to output open loops to prevent repeated points in point list.
                firstpoint=points[0]
                lastpoint=points[-1]
            ...
            np.savetxt("%s/%s-%i.csv"%(outfolder,outfile,number) , points,newline='\r\n')
